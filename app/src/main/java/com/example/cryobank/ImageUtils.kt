...
object ImageUtils {
    ...
    fun flipHorizontal(bmp: Bitmap): Bitmap {
        val matrix = Matrix().apply {
            preScale(-1f, 1f)   // horizontal spiegeln
        }
    ...
    fun majorityConsensus(perScanMaps: List<Map<String, String?>>): Map<String, String?> {
    ...
    /** Annotate rack image */
    // IMPORTANT:
// This function is VISUAL ONLY.
// It flips the image to match standard 96-well orientation.
// It must NEVER be used for decoding or Excel logic.

    fun annotateRackVisualOnly(fullImage: Bitmap, decodeMap: Map<String, String?>, highlightWell: String? = null): Bitmap {
        ...
        val paintText = Paint().apply {
            color = Color.WHITE
            textSize = (annotated.width / 36).toFloat()
        ...
        val rowsN = 8
        val colsN = 12
        val cellWf = annotated.width.toFloat() / colsN
        val cellHf = annotated.height.toFloat() / rowsN
        val insetPct = 0.05f
        ...
        for (ri in 0 until rowsN) {
            ...
            for (colIdx in 0 until colsN) {
                ...
                val xF = colIdx * cellWf
                val yF = ri * cellHf
                val left = xF + cellWf * insetPct
                val top = yF + cellHf * insetPct
                val right = xF + cellWf - cellWf * insetPct
                val bottom = yF + cellHf - cellHf * insetPct

                val well = "$r$colNumber"
                val code = decodeMap[well]
                paintRect.color = when {
                    highlightWell != null && highlightWell.equals(well, ignoreCase = true) -> Color.YELLOW
                    code != null -> Color.GREEN
                    else -> Color.RED
                }

                canvas.drawRect(left, top, right, bottom, paintRect)
                canvas.drawText(well, left + 6f, top + paintText.textSize + 6f, paintText)
            }
        }
        return annotated
    }
    ...
    fun decodeRackWellByWell(rackImage: Bitmap, ctx: Context? = null): Map<String, String?> {
        ...
        for (ri in 0 until rowsN) {
            ...
            for (colIdx in 0 until colsN) {
                ...
                val code = kotlinx.coroutines.runBlocking {
                    BarcodeDecoder.decodeCell(cellBmp)
                }
                val well = "$r$colNumber"
                map[well] = code
            }
    ...
    fun imageProxyToBitmapCorrected(image: ImageProxy): Bitmap {
        ...
        // Korrigiere Rotation, falls nötig
        val matrix = Matrix()
